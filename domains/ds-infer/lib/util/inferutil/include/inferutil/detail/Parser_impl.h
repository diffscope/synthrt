...
namespace ds::infer::inferutil {
    ...
    namespace detail {
        ...
        public:
            ...
            inline static constexpr std::array<std::pair<std::string_view, ParamTag>, 5>
                varianceMapping = {
                    std::pair{"energy",        srt::svs::Api::Common::L1::Tags::Energy      },
                    std::pair{"breathiness",   srt::svs::Api::Common::L1::Tags::Breathiness },
                    std::pair{"voicing",       srt::svs::Api::Common::L1::Tags::Voicing     },
                    std::pair{"tension",       srt::svs::Api::Common::L1::Tags::Tension     },
                    std::pair{"mouth_opening", srt::svs::Api::Common::L1::Tags::MouthOpening},
            };
            ...
            inline static constexpr std::array<std::pair<std::string_view, ParamTag>, 3>
                transitionMapping = {
                    std::pair{"gender",   srt::svs::Api::Common::L1::Tags::Gender  },
                    std::pair{"velocity", srt::svs::Api::Common::L1::Tags::Velocity},
            };
        ...
        inline void parse_parameters_common(Container &out, const std::string &fieldName,
                                            const srt::core::JsonObject &obj, ErrorCollector *ec) {
    ...
    inline void ConfigurationParser::parse_path_required(std::filesystem::path &out,
        ...
        if (const auto it = config.find(fieldName); it != config.end()) {
            if (!it->second.isString()) {
                ...
                out = stdc::path::clean_path(m_spec->path() /
    ...
    inline void ConfigurationParser::parse_phonemes(std::map<std::string, int> &out) {
        ...
        if (const auto it = config.find("phonemes"); it != config.end()) {
            if (!it->second.isString()) {
                ...
                auto path = m_spec->path() / stdc::path::from_utf8(it->second.toStringView());
            ...
            if (!it->second.isObject()) {
                ...
                for (const auto &[key, value] : obj) {
                    if (!value.isString()) {
                        ...
                        auto path = stdc::clean_path(m_spec->path() /
    ...
    inline void ConfigurationParser::parse_parameters(std::set<ParamTag> &out,
                                                      const std::string &fieldName) {
        const auto &obj = *m_pConfig;

        detail::parse_parameters_common<PT, std::set<ParamTag>>(out, fieldName, obj, m_ec);
    }
    ...
    inline void ConfigurationParser::parse_parameters(std::vector<ParamTag> &out,
        ...
        detail::parse_parameters_common<PT, std::vector<ParamTag>>(out, fieldName, obj, m_ec);
    }
    ...
    inline bool ConfigurationParser::loadIdMapping(const std::string &fieldName,
        ...
        auto j = srt::core::JsonValue::fromJson(buffer, true, &errString);
        if (!errString.empty()) {
            if (m_ec) {
                m_ec->collectError(std::move(errString));
            }
            return false;
        }
    ...
    inline void SchemaParser::parse_parameters(std::set<ParamTag> &out,
        ...
        detail::parse_parameters_common<PT, std::set<ParamTag>>(out, fieldName, schema, m_ec);
    }
    ...
    inline void SchemaParser::parse_parameters(std::vector<ParamTag> &out,
        ...
        detail::parse_parameters_common<PT, std::vector<ParamTag>>(out, fieldName, schema, m_ec);
    }
    ...
    inline void ImportOptionsParser::parse_parameters(std::set<ParamTag> &out,
        ...
        detail::parse_parameters_common<PT, std::set<ParamTag>>(out, fieldName, options, m_ec);
    }
    ...
    inline void ImportOptionsParser::parse_parameters(std::vector<ParamTag> &out,
        ...
        detail::parse_parameters_common<PT, std::vector<ParamTag>>(out, fieldName, options, m_ec);
    }
}
