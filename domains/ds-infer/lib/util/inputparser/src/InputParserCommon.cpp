...
namespace ds::infer {
        ...
        if (isDynamic) {
            ...
            if (auto it_value = parameter.find("value"); it_value != parameter.end()) {
                if (!it_value->second.isNumber()) {
                    return srt::core::Error(srt::core::Error::InvalidFormat, "for non-dynamic parameter, " +
                                                                     paramName +
    ...
    static inline int extractCents(double value) {
        ...
        int cents = static_cast<int>(integerPart) % 100;
        if (cents < 0) cents += 100;  // 处理负数输入
    ...
    srt::core::Expected<void> parseWords(const srt::core::JsonObject &obj,
        ...
        if (auto it_words = obj.find("words"); it_words != obj.end()) {
            if (!it_words->second.isArray()) {
                return srt::core::Error(srt::core::Error::InvalidFormat, "words must be an array");
            }
            ...
            for (const auto &maybeWord : words) {
                ...
                if (auto it_notes = word.find("notes"); it_notes != word.end()) {
                    ...
                    for (const auto &maybeNote : notes) {
                        ...
                        {
                            int offset = static_cast<int>(std::floor((noteInfo.cents + 50) / 100.0));
                            noteInfo.key += offset;
    ...
    srt::core::Expected<void> parseParameters(const srt::core::JsonObject &obj, bool pitchOnly,
        ...
        if (auto it_parameters = obj.find("parameters"); it_parameters != obj.end()) {
            if (!it_parameters->second.isArray()) {
                return srt::core::Error(srt::core::Error::InvalidFormat, "parameters must be an array");
            }
    ...
    srt::core::Expected<void> parseSpeakers(const srt::core::JsonObject &obj,
        ...
        if (auto it_speakers = obj.find("speakers"); it_speakers != obj.end()) {
            if (!it_speakers->second.isArray()) {
                return srt::core::Error(srt::core::Error::InvalidFormat, "speakers must be an array");
            }
            const auto &speakers = it_speakers->second.toArray();
