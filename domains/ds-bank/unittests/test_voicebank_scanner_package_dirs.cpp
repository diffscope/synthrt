...
TEST_CASE("VoicebankScanner packageDirectories returns all versions",
    ...
    for (const auto &d : dirs) {
        pathStrs.insert(d.path.lexically_normal().string());
    }
    REQUIRE(pathStrs.count(pkgDir1.lexically_normal().string()) == 1);
    REQUIRE(pathStrs.count(pkgDir2.lexically_normal().string()) == 1);
