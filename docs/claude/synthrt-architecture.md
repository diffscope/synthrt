...
| IID | 接口 | 谁实现 |
|---|---|---|
| `org.openvpi.synthrt.plugin.InferenceInterpreter` | `InferenceInterpreterPlugin` | dsinfer：acoustic / duration / pitch / variance / vocoder |
| `org.openvpi.synthrt.plugin.SingerProvider` | `SingerProviderPlugin` | dsinfer：diffsinger |
| `org.openvpi.dsinfer.plugin.InferenceDriver` | `InferenceDriverPlugin`（**定义在 dsinfer**） | dsinfer：onnxdriver |
| `org.openvpi.wolf.plugin.LanguageInterpreter` | `LanguageInterpreterPlugin`（**定义在 wolf**） | wolf：cmn |
