...
消费者按用户的语言偏好挑选，规则是 **RFC 4647 的 Lookup**：用偏好标签去找，找不到就从右端逐段去掉子标签再找，直到只剩语言那一段，仍然找不到则取`_`。

偏好`zh-Hant-TW`时依次尝试`zh-Hant-TW`、`zh-Hant`、`zh`，最后`_`。

标签匹配**不区分大小写**。书写建议遵循 BCP 47 的惯例：语言小写，文字子标签首字母大写，地区全大写，如`zh-Hans-CN`。

> 2.3 中这里写的是`zh_CN`、`ja_JP`，那是 POSIX 的 locale 写法。改用 BCP 47 有两个原因：分隔符本应是连字符，而且 POSIX 那套表达不了文字子标签，区分不了`zh-Hans`与`zh-Hant`。

#### 用在哪些字段

`desc.json`中是`vendor`、`copyright`、`description`，模块声明文件中是`name`。贡献类别与契约追加的字段是否多语言，由定义它们的一方规定。
