...
namespace srt::driver {
    ...
    srt::core::Expected<void> setupOnnxInferenceDriver(srt::core::Runtime          &runtime,
                                                       const std::filesystem::path &pluginRoot,
                                                       const OnnxDriverConfig      &config) {
        const auto driverPluginDir =
            pluginRoot / stdc::path::from_utf8("srt-driver") / stdc::path::from_utf8("inferencedrivers");
        const auto diffsingerPluginDir = pluginRoot / stdc::path::from_utf8("diffsinger");
        const auto singerProviderDir   = diffsingerPluginDir / stdc::path::from_utf8("singerproviders");
        const auto interpreterDir      = diffsingerPluginDir / stdc::path::from_utf8("inferenceinterpreters");
        ...
        auto onnxArgs         = srt::core::NO<srt::driver::onnx::DriverInitArgs>::create();
        ...
        const auto ortParentPath =
            plugin->path().parent_path() / stdc::path::from_utf8("runtimes") / stdc::path::from_utf8("onnx");
