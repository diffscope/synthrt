...
namespace dsinfer_cli {
    ...
    InputObject buildInputFromPiece(srt::g2p::LanguageService &langSvc, const ds::bank::SingerRef &ref,
                                    const MidiPiece &piece, const std::string &speakerId, const std::string &languageId,
        ...
        auto routeExp = langSvc.resolveLanguageRoute(ref.packageId, g2pContextVersion, ref.singerId, languageId);
        if (routeExp) {
        ...
        if (!s2pMode.empty()) {
            try {
                if (s2pMode == "dict" && !s2pFile.empty()) {
                    ...
                    langResource = std::unique_ptr<srt::s2p::LanguageResource>(new srt::s2p::LanguageResource(
                        srt::s2p::LanguageResource::direct(stdc::path::to_utf8(onsetFile))));
                }
            } catch (const std::exception &e) {
                throw std::runtime_error(stdc::formatN("S2P resource construction failed (s2pMode=%1, s2pFile=%2): %3",
        ...
        for (const auto &note : piece.notes) {
            ...
            if (!nd.isRest && !nd.isSlur) {
                ...
                if (g2pResults.empty()) {
                    throw std::runtime_error(stdc::formatN("G2P returned empty results for lyric '%1' (g2pId=%2)",
                ...
                if (g2pRes.isFailed()) {
                    throw std::runtime_error(stdc::formatN("G2P inference failed for lyric '%1' (errorType=%2)",
                                                           note.lyric, static_cast<int>(g2pRes.errorType)));
                }
                if (g2pRes.pronunciation.empty()) {
                    throw std::runtime_error(
                        stdc::formatN("G2P returned empty pronunciation for lyric '%1'", note.lyric));
                }
                ...
                if (g2pRes.mode == srt::g2p::kG2pModeCopy && g2pRes.pronunciation == note.lyric &&
                    ...
                    throw std::runtime_error(
                        stdc::formatN("G2P returned copy fallback for lyric '%1' (context not Ready, g2pId=%2)",
                                      note.lyric, effectiveG2pId));
                }
                ...
                if (!langResource) {
                    throw std::runtime_error(stdc::formatN("S2P resource not available (s2pMode='%1', s2pFile='%2')",
                ...
                try {
                    ...
                    throw std::runtime_error(
                        stdc::formatN("S2P conversion failed for pronunciation '%1': %2", pronunciationText, e.what()));
                }
                if (pron.phonemes.empty()) {
                    throw std::runtime_error(
                        stdc::formatN("S2P returned empty phonemes for pronunciation '%1'", pronunciationText));
                }
