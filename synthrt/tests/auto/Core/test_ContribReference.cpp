...
using srt::ContribReference;

BOOST_AUTO_TEST_SUITE(test_ContribReference)

BOOST_AUTO_TEST_CASE(test_parse_and_render) {
    const auto local = ContribReference::fromString(":singer/main");
    ...
    const auto external = ContribReference::fromString("vendor/sample:com.example.category/item-1");
...
BOOST_AUTO_TEST_CASE(test_rejects_incomplete_and_non_ascii_references) {
    for (const auto *value : {"", "singer/main", ":singer", ":/main", ":singer/",
                              ":singer/main/extra", "foo::singer/main", ":singer/声音"}) {
        BOOST_CHECK_MESSAGE(!ContribReference::fromString(value).isValid(), value);
    }
}
